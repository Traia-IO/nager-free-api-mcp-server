#!/usr/bin/env python3
"""
Nager Free API MCP Server - FastMCP with D402 Transport Wrapper
...
Generated from OpenAPI: http://date.nager.at/docs
...
logger = logging.getLogger('nager-free-api_mcp')
...
logger.info("="*80)
logger.info(f"Nager Free API MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: http://date.nager.at/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager Free API MCP Server", host="0.0.0.0")
...
# - NAGER_FREE_API_API_KEY: Server's internal API key for payment mode
#
...
async def retrieves_detailed_information_about_a_specific_country(
    ...
    try:
        url = f"http://date.nager.at/api/v3/CountryInfo/{countryCode}"
        params = {}
        headers = {}
...
async def retrieve_the_complete_list_of_all_countries_supported_by_the_nagerdate_api(
    ...
    try:
        url = f"http://date.nager.at/api/v3/AvailableCountries"
        params = {}
        headers = {}
        # No auth required for this API
...
async def retrieve_all_long_weekends_for_a_given_country_and_year(
    ...
    try:
        url = f"http://date.nager.at/api/v3/LongWeekend/{year}/{countryCode}"
        params = {
...
async def retrieve_the_list_of_all_public_holidays_for_the_specified_year_and_country(
    ...
    try:
        url = f"http://date.nager.at/api/v3/PublicHolidays/{year}/{countryCode}"
        params = {}
        headers = {}
        # No auth required for this API
...
async def retrieve_all_upcoming_public_holidays_occurring_within_the_next_365_days_for_a_given_country(
    ...
    try:
        url = f"http://date.nager.at/api/v3/NextPublicHolidays/{countryCode}"
        params = {}
        headers = {}
        # No auth required for this API
...
async def retrieve_all_public_holidays_occurring_worldwide_within_the_next_7_days(
    ...
    try:
        url = f"http://date.nager.at/api/v3/NextPublicHolidaysWorldwide"
        params = {}
        headers = {}
        # No auth required for this API
...
async def retrieve_the_current_version_information_of_the_nagerdate_library(
    ...
    try:
        url = f"http://date.nager.at/api/v3/Version"
        params = {}
        headers = {}
        # No auth required for this API
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nager-free-api-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nager-free-api-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager Free API MCP Server")
    logger.info("="*80)
